...
export default function Verification() {
  ...
  return (
    <section style={{ backgroundImage: `url(${myImage})` }} className="min-h-screen bg-center bg-cover flex items-center justify-center relative py-10 lg:py-0">
      ...
      <div className='w-full pt-20 px-4 lg:px-0'>
        <form 
        ...
        >
          <div className="mb-6">
            ...
            <p className='text-gray-600 text-center font-normal text-lg lg:text-[24px] mb-2 px-2'>
              Enter the code from the email we sent <br className='hidden lg:block' /> to <span className='text-black font-semibold break-all'>{email}</span>
            </p>
          </div>
          ...
          <OtpInput
            ...
            renderInput={(props) => (
              <input
                {...props}
                type="text"
                className="w-14! h-14! lg:w-20! lg:h-20! text-2xl lg:text-3xl font-bold text-white bg-[#036464] rounded-[15px] lg:rounded-[20px] outline-none focus:ring-4 focus:ring-cyan-500 transition-all shadow-lg"
              />
            )}
          />
          </div>
